...
class XmlAssembler {
  ...
  define(ident, expr) {
    ...
    if (frame._scopeType != Scope.OBJECT) {
      throw new Error(`Internal error: cannot define a member on a ${frame._scopeType} context`);
    }
  ...
  beginCondition(ident, expr) {
    ...
    const frame = this.contextStack;
    if (frame._scopeType != Scope.OBJECT) {
      throw new Error(`Internal error: cannot define a condition on a ${frame._scopeType} context`);
    }
    const evaluator = Engine.compileExpr(expr); // these are cached so this should be fast
  ...
  beginList(ident, expr) {
    const frame = this.contextStack;
    const evaluator = Engine.compileExpr(expr); // these are cached so this should be fast
    let iterable = frame._evaluate(evaluator); // we need to make sure this is memoized to avoid unnecessary re-evaluation
