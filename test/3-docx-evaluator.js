...
describe('Generating XML data for DOCX templates (white box)', function () {
  it('auto-generated json file should allow execution with an empty context', async function () {
    const templatePath = testUtil.GetTemplatePath('SimpleWill.docx')
    let jsonFile = templatePath + '.json'
    ...
    if (!fs.existsSync(jsonFile)) {
      ...
      assert.strictEqual(fs.existsSync(result.ExtractedLogicTree), true)
      assert.strictEqual(fs.existsSync(result.DocxGenTemplate), true)
      jsonFile = result.ExtractedLogicTree
    }
    const str = new XmlAssembler({}).assembleXml(jsonFile)
  ...
  it('auto-generated json file should allow execution against its contextHelper with a populated context', async function () {
    const templatePath = testUtil.GetTemplatePath('SimpleWill.docx')
    let jsonFile = templatePath + '.json'
    // only re-compile if necessary -- should only happen if this test file is being run independently of others
    if (!fs.existsSync(jsonFile)) {
    ...
    const str = new XmlAssembler(data).assembleXml(jsonFile)
  ...
  it('list testing', async function () {
    ...
    // now evaluate the helper against this data context, to test its functionality
    const str = new XmlAssembler(data).assembleXml(jsFile) // should work with either .json OR old .js file path!
    fs.writeFileSync(templatePath + '.asmdata.xml', str)
  ...
  it('should produce usable XML when an unconditional usage of a variable follows a conditional one', async function () {
    ...
    fs.writeFileSync(templatePath + '.asmdata.xml', str)
    const asmResult = await openDocx.assembleDocx(templatePath, templatePath + '-assembled.docx', data)
  ...
  it('should produce usable XML and a valid assembled document for a simple "if x then x" template', async function () {
    ...
    let asmResult = await openDocx.assembleDocx(templatePath, templatePath + '-assembled1.docx', data)
    assert(!asmResult.HasErrors)
    ...
    asmResult = await openDocx.assembleDocx(templatePath, templatePath + '-assembled2.docx', {})
    assert(!asmResult.HasErrors)
  })
  ...
  it('should create the expected XML for BeneficiaryList.docx', async function () {
    ...
    assert.strictEqual(str, expectedXml)
    fs.writeFileSync(templatePath + '.asmdata2.xml', str)
    asmResult = await openDocx.assembleDocx(templatePath, templatePath + '-assembled2.docx', data)
    assert(!asmResult.HasErrors)
  })
})
